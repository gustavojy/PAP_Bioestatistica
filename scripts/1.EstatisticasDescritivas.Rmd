...
knitr::include_graphics("~/GitHub/PAP_Bioestatistica/figuras/1.EstDescritiva/med_resumo.png")
```
...
No R, utilizamos a função `IQR()` para calcular a distância interquantil. Da mesma forma que a função `quantile()`, podemos selecionar o tipo de algoritmo adotado a partir do argumento `type =`.  

```{r}
# Distância interquantil
IQR(dados$altura, type = 3)
```
...
Para calcular a variância e o desvio padrão de multiplas variáveis, usa-se a função `purrr::map()`, tendo como argumento o tipo de operação a ser realizada (nesse caso, `sd` para desvio padrão ou `var` para a variância). Perceba que é atribuído valores ausentes (`NA`) para as variáveis categóricas.  
...
knitr::include_graphics("~/GitHub/PAP_Bioestatistica/figuras/1.EstDescritiva/exploratoria_1.png")
```
