...
Seja $acertar = p(x=1) = 1/5$ e $errar = p(x=0)$, temos pela definição que:  
...
# Cálculo de Binomial
## Resolvendo o exercício com a função dbinom()
(binomial_90_100 <- dbinom(x = 90:100, size = 100, prob = 0.8864))
```

Como resultado, devemos somar os resultados obtidos anteriormente
...
Ainda, podemos criar um gráfico para todas as probabilidades possiveis:  

```{r}
# Todas as probabilidades do exercício anterior
p_0_100 <- dbinom(x = 0:100, size = 100, prob = 0.5)

tibble(px = 0:100,
       prob = p_0_100 %>% round(4)) %>% 
